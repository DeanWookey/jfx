...
public class CSSStyleHelperPseudoClassTest {
    ...
    @Test
    public void testMovedNodeGetsCorrectPseudoClassState() {
        Stylesheet stylesheet = null;
        root.getStyleClass().add("root");
        try {
            stylesheet = new CssParser().parse(
                    "testFreshNodeGetsCorrectPseudoClassState",
        ...
        Toolkit.getToolkit().firePulse();
        assertEquals(Color.YELLOW, redPane.backgroundProperty().getValue().getFills().get(0).getFill()); //changes to yellow after being moved to a different part of the scene graph
        ...
        Toolkit.getToolkit().firePulse();
        assertEquals(Color.YELLOW, yellowPane.backgroundProperty().getValue().getFills().get(0).getFill()); //when first inserted, should be yellow

        root.getChildren().add(yellowPane);
        Toolkit.getToolkit().firePulse();
        assertEquals(Color.RED, yellowPane.backgroundProperty().getValue().getFills().get(0).getFill()); //changes to red after being moved to a different part of the scene graph
